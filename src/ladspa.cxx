...
std::shared_ptr<ladspa::instance> ladspa::instantiate(const id_type id_in, const size_type sample_rate_in)
{
    return get_type(id_in)->instantiate(sample_rate_in);
}
...
std::shared_ptr<ladspa::instance> ladspa::type::instantiate(const ladspa::size_type sample_rate_in)
{
    auto new_handle = descriptor->instantiate(descriptor, sample_rate_in);
    return std::make_shared<ladspa::instance>(new_handle, this);
}
...
ladspa::data_type ladspa::instance::get_control(const ladspa::id_type id_in)
{
    ...
    assert(type->get_port(id_in)->is_output());
...
ladspa::instance::instance(const LADSPA_Handle handle_in, ladspa::type * type_in)
